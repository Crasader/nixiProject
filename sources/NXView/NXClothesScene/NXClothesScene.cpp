...
NXClothesScene* NXClothesScene::create_with_mystery(int _type_id, const char *category, const char *tishi) {
    NXClothesScene* rtn = NXClothesScene::create();
    rtn->init_with_mystery(_type_id, category, tishi);
    
    return rtn;
}

void NXClothesScene::init_with_mystery(int _type_id, const char *category, const char *tishi) {
    this->category = category;
    this->tishi = tishi;
    this->init_with_type(_type_id, 1, 1);
}

NXClothesScene* NXClothesScene::create_with_tryst(const char *tishi) {
    NXClothesScene* rtn = NXClothesScene::create();
...
void NXClothesScene::init_with_tryst(const char *tishi) {
    this->tishi = tishi;
    this->init_with_type(4, 1, 1);
}

NXClothesScene* NXClothesScene::create_with_type(int _type_id, int _task_index, int _task_phase){
    NXClothesScene* rtn = NXClothesScene::create();
...
void NXClothesScene::init_with_type(int _type_id, int _task_index, int _task_phase){
...
//    bgSpr = CCSprite::create("res/nxpic/NXClothesShop/NXClothesShop_bj.png");
    ...
    CCMenuItem* backItem = CCMenuItemSprite::create(backSpr1, backSpr2, this, menu_selector(NXClothesScene::backCallBack));
...
//        saveItem = CCMenuItemSprite::create(startSpr1, startSpr2, startSpr3, this, menu_selector(NXClothesScene::saveCallBack));
...
//        startSpr2->setScale(1.02f);
//        startItem = CCMenuItemSprite::create(startSpr1, startSpr2, this, menu_selector(NXClothesScene::startCallBack));
...
//        shareSpr2->setScale(1.02f);
//    }
//    shareItem = CCMenuItemSprite::create(shareSpr1, shareSpr2, this, menu_selector(NXClothesScene::shareCallBack));
...
//    this->scheduleOnce(SEL_SCHEDULE(&NXClothesScene::openButtonMenu), .1f);
    
    int randIndex = 1 + rand()%2;
    if (randIndex == 2) {
        this->scheduleOnce(SEL_SCHEDULE(&NXClothesScene::randomHint), 2.f);
    }
    
}
void NXClothesScene::randomHint(float dt){
...
void NXClothesScene::shareCallBack(CCObject* pSender){
    ...
    this->schedule(SEL_SCHEDULE(&NXClothesScene::shareStatus), .1f);
#endif
}
void NXClothesScene::shareStatus(float dt){
    ...
    if (JNIController::getShareStatus() == 1) {
        ...
        NET->daily_share_321();
        CCLog("《《《《《《 分享成功 》》》》》》》");
        this->unschedule(SEL_SCHEDULE(&NXClothesScene::shareStatus));
...
void NXClothesScene::_321CallBack(CCObject* pSender){
    LOADING->remove();
}
void NXClothesScene::iOS_share_finish(CCObject* pSender){
    LOADING->remove();
}


bool NXClothesScene::init(){
    if (!NXBaseScene::init()) {
        return false;
    }
    num_child = 0;
...
void NXClothesScene::onEnter(){
    NXBaseScene::onEnter();
    NXBaseScene::openChat(false);
    
    CCNotificationCenter* nc = CCNotificationCenter::sharedNotificationCenter();
    nc->addObserver(this, menu_selector(NXClothesScene::ChangeClothes), "ChangeClothes", NULL);
    nc->addObserver(this, menu_selector(NXClothesScene::ChangClothesIndex), "ChangClothesIndex", NULL);
    nc->addObserver(this, menu_selector(NXClothesScene::updataSaveItemStatus), "UpdataSaveItemStatus", NULL);
    nc->addObserver(this, menu_selector(NXClothesScene::backCallBack), "GuideBackCallBack", NULL);
    
    nc->addObserver(this, menu_selector(NXClothesScene::buttonStatus), "ButtonStatus", NULL);
    nc->addObserver(this, menu_selector(NXClothesScene::renwukuangCallBack), "GuideRenwukuangCallBack", NULL);
    nc->addObserver(this, menu_selector(NXClothesScene::guideButtonCallBack), "GuideButtonCallBack", NULL);
    nc->addObserver(this, menu_selector(NXClothesScene::startCallBack), "GuideStartCallBack", NULL);
    nc->addObserver(this, menu_selector(NXClothesScene::removeAnimation), "GuideRemoveAnimation", NULL);
    nc->addObserver(this, menu_selector(NXClothesScene::buyCallBack), "GuideBuyCallBack", NULL);
    
    
    nc->addObserver(this, menu_selector(NXClothesScene::_905CallBack), "HTTP_FINISHED_905", NULL);
    
    
    nc->addObserver(this, menu_selector(NXClothesScene::Http_Finished_401), "HTTP_FINISHED_401", NULL);
    nc->addObserver(this, menu_selector(NXClothesScene::Http_Finished_401), "HTTP_FINISHED_403", NULL);
    nc->addObserver(this, menu_selector(NXClothesScene::Http_Finished_401), "HTTP_FINISHED_405", NULL);
    
    //    nc->addObserver(this, menu_selector(NXClothesScene::Http_Finished_601), "HTTP_FINISHED_601", NULL);
    nc->addObserver(this, menu_selector(NXClothesScene::Http_Finished_603), "HTTP_FINISHED_603", NULL);
    nc->addObserver(this, menu_selector(NXClothesScene::_605CallBack), "HTTP_FINISHED_605", NULL);
    
    nc->addObserver(this, menu_selector(NXClothesScene::after_commit_mystery_613), "HTTP_FINISHED_613", NULL);
    nc->addObserver(this, menu_selector(NXClothesScene::after_start_tryst_task_623), "HTTP_FINISHED_623", NULL);
    nc->addObserver(this, menu_selector(NXClothesScene::_321CallBack), "HTTP_FINISHED_321", NULL);
...
void NXClothesScene::onExitTransitionDidStart() {
    DISPLAY->stopBlink();
}

void NXClothesScene::onExit(){
...
void NXClothesScene::keyBackClicked(){
    num_child++;
...
bool NXClothesScene::ccTouchBegan(CCTouch * pTouch, CCEvent * pEvent){
    if (animationBool) {
        animationBool = false;
...
void NXClothesScene::tuoguangConfirmCallBack(CCObject* pSender){
...
//    }
    ...
    if (isThereAClothes()) {// 有穿衣服
        ...
        if (buttonTag != Tag_NXCL_ZhuangRong && buttonTag != Tag_NXCL_TeXiao) {
        ...
        }else if (buttonTag == Tag_NXCL_ZhuangRong){
            if (clothKuangSpr->getChildren() != NULL) {
                clothKuangSpr->removeAllChildren();
            }
...
bool NXClothesScene::isThereAClothes(){
    bool _bool = false;
    CCDictionary* myClothesTempDic = DATA->getClothes()->MyClothesTemp();
    CCDictionary* shipinDic;
    for (int i = Tag_NXCL_TouFa; i <= Tag_NXCL_ZhuangRong; i++) {
...
void NXClothesScene::tuoguangCancelCallBack(CCObject* pSender){
    if (this->getChildByTag(0x8888) != NULL) {
        tskSpr->removeAllChildren();
        this->removeChildByTag(0x8888);
    }
...
cocos2d::CCScene* NXClothesScene::scene(){
    CCScene* scene = CCScene::create();
    NXClothesScene* layer = NXClothesScene::create();
    scene->addChild(layer);
    
    return scene;
}

void NXClothesScene::openTouch(float dt){
    
}

void NXClothesScene::creat_View(){
    if (this->getChildByTag(0x2222) != NULL) {
        this->removeChildByTag(0x2222);
    }
    
    // 试衣间底框
    clothKuangSpr = CCSprite::create("res/nxpic/NXClothes/NXClothes_bgkuang.png");
    clothKuangSpr->setAnchorPoint(ccp(1, 0.f));
    clothKuangSpr->setPosition(ccp(DISPLAY->ScreenWidth() - 5.f, 4.f));
    clothKuangSpr->setTag(0x2222);
    this->addChild(clothKuangSpr, 15);
    
    CCSprite* shopSpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_shop.png");
    CCSprite* shopSpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_shop.png");
    shopSpr2->setScale(1.02f);
    shopItem = CCMenuItemSprite::create(shopSpr1, shopSpr2, this, menu_selector(NXClothesScene::shopCallback));
    shopItem->setAnchorPoint(ccp(1.f, 0.f));
    shopItem->setPosition(ccp(-2.f, 2.f));
    CCMenu* shopMenu = CCMenu::create(shopItem, NULL);
    shopMenu->setPosition(CCPointZero);
    clothKuangSpr->addChild(shopMenu, 15);
    
    CCSprite* dikuangSpr = CCSprite::create("res/nxpic/NXClothes/NXClothes_dikuang.png");
    dikuangSpr->setAnchorPoint(ccp(.5f, 0.f));
    dikuangSpr->setPosition(ccp(clothKuangSpr->getContentSize().width* .5f, 0.f));
    clothKuangSpr->addChild(dikuangSpr, 10);
    // 保存
    CCSprite* startSpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_save.png");
    CCSprite* startSpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_save.png");
    startSpr2->setScale(1.02f);
    CCSprite* startSpr3 = CCSprite::create("res/nxpic/NXClothes/NXClothes_save.png");
    startSpr3->setColor(ccGRAY);
    saveItem = CCMenuItemSprite::create(startSpr1, startSpr2, startSpr3, this, menu_selector(NXClothesScene::saveCallBack));
    saveItem->setAnchorPoint(ccp(.5f, .5f));
    saveItem->setPosition(ccp(dikuangSpr->getContentSize().width* .5f, dikuangSpr->getContentSize().height* .45f));
    CCMenu* saveMenu = CCMenu::create(saveItem, NULL);
    saveMenu->setPosition(CCPointZero);
    dikuangSpr->addChild(saveMenu);
    ...
    CCMenuItem* buyItem = CCMenuItemSprite::create(buySpr1, buySpr2, this, menu_selector(NXClothesScene::buyCallBack));
    ...
    CCSprite* tfSpr1;
    CCSprite* tfSpr2;
    if (buttonTag == Tag_NXCL_TouFa) {
        tfSpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_selected.png");
        tfSpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_selected.png");
        tfItem = CCMenuItemSprite::create(tfSpr1, tfSpr2, this, NULL);
    }else{
        tfSpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_tf1.png");
        tfSpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_tf1.png");
        tfSpr2->setScale(1.01f);
        tfItem = CCMenuItemSprite::create(tfSpr1, tfSpr2, this, menu_selector(NXClothesScene::buttonCallBack));
    }
    tfItem->setTag(Tag_NXCL_TouFa);
    CCSprite* tfSpr = CCSprite::create("res/nxpic/NXClothes/NXClothes_tf2.png");
...
//    tfSpr->addChild(ycSpr1);
    ...
    CCSprite* wtSpr1;
    CCSprite* wtSpr2;
    if (buttonTag == Tag_NXCL_WaiTao) {
        wtSpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_selected.png");
        wtSpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_selected.png");
        wtItem = CCMenuItemSprite::create(wtSpr1, wtSpr2, this, NULL);
    }else{
        wtSpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_wt1.png");
        wtSpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_wt1.png");
        wtSpr2->setScale(1.01f);
        wtItem = CCMenuItemSprite::create(wtSpr1, wtSpr2, this, menu_selector(NXClothesScene::buttonCallBack));
    }
    wtItem->setTag(Tag_NXCL_WaiTao);
    CCSprite* wtSpr = CCSprite::create("res/nxpic/NXClothes/NXClothes_wt2.png");
...
//    wtSpr->addChild(ycSpr2);
    ...
    CCSprite* sySpr1;
    CCSprite* sySpr2;
    if (buttonTag == Tag_NXCL_ShangYi) {
        sySpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_selected.png");
        sySpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_selected.png");
        syItem = CCMenuItemSprite::create(sySpr1, sySpr2, this, NULL);
    }else{
        sySpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_sy1.png");
        sySpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_sy1.png");
        sySpr2->setScale(1.01f);
        syItem = CCMenuItemSprite::create(sySpr1, sySpr2, this, menu_selector(NXClothesScene::buttonCallBack));
    }
    syItem->setTag(Tag_NXCL_ShangYi);
    CCSprite* sySpr = CCSprite::create("res/nxpic/NXClothes/NXClothes_sy2.png");
...
//    sySpr->addChild(ycSpr3);
    ...
    CCSprite* kzSpr1;
    CCSprite* kzSpr2;
    if (buttonTag == Tag_NXCL_KuZi) {
        kzSpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_selected.png");
        kzSpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_selected.png");
        kzItem = CCMenuItemSprite::create(kzSpr1, kzSpr2, this, NULL);
    }else{
        kzSpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_kz1.png");
        kzSpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_kz1.png");
        kzSpr2->setScale(1.01f);
        kzItem = CCMenuItemSprite::create(kzSpr1, kzSpr2, this, menu_selector(NXClothesScene::buttonCallBack));
    }
    kzItem->setTag(Tag_NXCL_KuZi);
    CCSprite* kzSpr = CCSprite::create("res/nxpic/NXClothes/NXClothes_kz2.png");
...
//        wzItem = CCMenuItemSprite::create(wzSpr1, wzSpr2, this, menu_selector(NXClothesScene::buttonCallBack));
//    }
//    wzItem->setTag(Tag_NXCL_WaZi);
...
//    wzSpr->addChild(ycSpr5);
    ...
    CCSprite* xzSpr1;
    CCSprite* xzSpr2;
    if (buttonTag == Tag_NXCL_XieZi) {
        xzSpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_selected.png");
        xzSpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_selected.png");
        xzItem = CCMenuItemSprite::create(xzSpr1, xzSpr2, this, NULL);
    }else{
        xzSpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_xz1.png");
        xzSpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_xz1.png");
        xzSpr2->setScale(1.01f);
        xzItem = CCMenuItemSprite::create(xzSpr1, xzSpr2, this, menu_selector(NXClothesScene::buttonCallBack));
    }
    xzItem->setTag(Tag_NXCL_XieZi);
    CCSprite* xzSpr = CCSprite::create("res/nxpic/NXClothes/NXClothes_xz2.png");
...
//    xzSpr->addChild(ycSpr6);
    ...
    CCSprite* spSpr1;
    CCSprite* spSpr2;
    if (buttonTag == Tag_NXCL_ShiPin) {
        spSpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_selected.png");
        spSpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_selected.png");
        spItem = CCMenuItemSprite::create(spSpr1, spSpr2, this, NULL);
    }else{
        spSpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_sp1.png");
        spSpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_sp1.png");
        spSpr2->setScale(1.01f);
        spItem = CCMenuItemSprite::create(spSpr1, spSpr2, this, menu_selector(NXClothesScene::buttonCallBack));
    }
    spItem->setTag(Tag_NXCL_ShiPin);
    CCSprite* spSpr = CCSprite::create("res/nxpic/NXClothes/NXClothes_sp2.png");
...
//        baoItem = CCMenuItemSprite::create(baoSpr1, baoSpr2, this, menu_selector(NXClothesScene::buttonCallBack));
//    }
//    baoItem->setTag(Tag_NXCL_Bao);
...
//    baoSpr->addChild(ycSpr8);
    ...
    if (buttonTag == Tag_NXCL_ZhuangRong) {
        zhuangrongSpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_selected.png");
        zhuangrongSpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_selected.png");
        zhuangrongItem = CCMenuItemSprite::create(zhuangrongSpr1, zhuangrongSpr2, this, NULL);
    }else{
        zhuangrongSpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_zr1.png");
        zhuangrongSpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_zr1.png");
        zhuangrongSpr2->setScale(1.01f);
        zhuangrongItem = CCMenuItemSprite::create(zhuangrongSpr1, zhuangrongSpr2, this, menu_selector(NXClothesScene::buttonCallBack));
    }
    zhuangrongItem->setTag(Tag_NXCL_ZhuangRong);
    CCSprite* zhuangrongSpr = CCSprite::create("res/nxpic/NXClothes/NXClothes_zr2.png");
...
//    zhuangrongSpr->addChild(ycSpr9);
    ...
    CCSprite* texiaoSpr1;
    CCSprite* texiaoSpr2;
    if (buttonTag == Tag_NXCL_TeXiao) {
        texiaoSpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_selected.png");
        texiaoSpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_selected.png");
        texiaoItem = CCMenuItemSprite::create(texiaoSpr1, texiaoSpr2, this, NULL);
    }else{
        texiaoSpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_tx1.png");
        texiaoSpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_tx1.png");
        texiaoSpr2->setScale(1.01f);
        texiaoItem = CCMenuItemSprite::create(texiaoSpr1, texiaoSpr2, this, menu_selector(NXClothesScene::buttonCallBack));
    }
    texiaoItem->setTag(Tag_NXCL_TeXiao);
    CCSprite* texiaoSpr = CCSprite::create("res/nxpic/NXClothes/NXClothes_tx2.png");
...
//    texiaoSpr->addChild(ycSpr10);
    ...
    buttonMenu = CCMenu::create(syItem, kzItem, wtItem, xzItem, texiaoItem, tfItem, spItem, zhuangrongItem, NULL);
    buttonMenu->alignItemsVerticallyWithPadding(15.f);
    buttonMenu->setPosition(ccp(- tfItem->getContentSize().width* .41f, clothKuangSpr->getContentSize().height* .545f));
...
//    tabLayer->setPosition(ccp(0, clothKuangSpr->getContentSize().height* .128f));
    ...
    if (buttonTag == Tag_NXCL_TouFa){
    ...
    }else if (buttonTag == Tag_NXCL_WaiTao){
        
        isClothesType = Tag_NXCL_WaiTao;
        if (renwukuangMethodsBool) {
            renwukuangMethods(isClothesType);
        }else{
            creat_ViewMethods(isClothesType);
        }
    }else if (buttonTag == Tag_NXCL_ShangYi){
        
        isClothesType = Tag_NXCL_ShangYi;
        if (renwukuangMethodsBool) {
            renwukuangMethods(isClothesType);
        }else{
            creat_ViewMethods(isClothesType);
        }
    }else if (buttonTag == Tag_NXCL_KuZi){
    ...
    }else if (buttonTag == Tag_NXCL_XieZi){
    ...
    }else if (buttonTag == Tag_NXCL_ShiPin){
        
        isClothesType = Tag_NXCL_ShiPin;
        if (renwukuangMethodsBool) {
            renwukuangMethods(isClothesType);
        }else{
            creat_ViewMethods(isClothesType);
        }
    }else if (buttonTag == Tag_NXCL_Bao){
        
        isClothesType = Tag_NXCL_Bao;
        if (renwukuangMethodsBool) {
            renwukuangMethods(isClothesType);
        }else{
            creat_ViewMethods(isClothesType);
        ...
        isClothesType = Tag_NXCL_ZhuangRong;
        if (renwukuangMethodsBool) {
            renwukuangMethods(isClothesType);
        }else{
            creat_ViewMethods(isClothesType);
...
void NXClothesScene::openButtonMenu(){
    buttonMenu->setEnabled(true);
}
void NXClothesScene::creat_ViewMethods(int index) {
    CCDictionary* dic = CONFIG->clothes();// 所有衣服
...
void NXClothesScene::crate_Tishi(){
...
//        this->addChild(renwukuangSpr, 10);
        ...
        CCMenuItem* renwukuangItem = CCMenuItemSprite::create(renwukuangSpr1, renwukuangSpr2, this, menu_selector(NXClothesScene::renwukuangCallBack));
        ...
        this->creatTuoguang(renwukuangItem);
    ...
    else if (clothesStatus == 2){
        CCSprite* tuoguangSpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_tuoguang.png");
        CCSprite* tuoguangSpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_tuoguang.png");
        tuoguangSpr2->setScale(1.02f);
        CCMenuItem* tuoguangItem = CCMenuItemSprite::create(tuoguangSpr1, tuoguangSpr2, this, menu_selector(NXClothesScene::tuoguangCallback));
        tuoguangItem->setAnchorPoint(ccp(0.f, .5f));
        tuoguangItem->setPosition(ccp(20.f, DISPLAY->ScreenHeight()* .8f));
        CCMenu* tuoguangMenu = CCMenu::create(tuoguangItem, NULL);
        tuoguangMenu->setPosition(CCPointZero);
        this->addChild(tuoguangMenu, 20);
    }
...
void NXClothesScene::creatTuoguang(CCMenuItem* item){
    CCSprite* tuoguangSpr1 = CCSprite::create("res/nxpic/NXClothes/NXClothes_tuoguang.png");
    CCSprite* tuoguangSpr2 = CCSprite::create("res/nxpic/NXClothes/NXClothes_tuoguang.png");
    tuoguangSpr2->setScale(1.02f);
    CCMenuItem* tuoguangItem = CCMenuItemSprite::create(tuoguangSpr1, tuoguangSpr2, this, menu_selector(NXClothesScene::tuoguangCallback));
    tuoguangItem->setAnchorPoint(ccp(0.f, 1.f));
    tuoguangItem->setPosition(ccp(0.f, 20.f));
    CCMenu* tuoguangMenu = CCMenu::create(tuoguangItem, NULL);
    tuoguangMenu->setPosition(CCPointZero);
    item->addChild(tuoguangMenu, 20);
}
void NXClothesScene::tuoguangCallback(CCObject* pSender){
    if (!zhuangrongBool) {
        _manBool = true;
    }else{
        _manBool = false;
    ...
    if (_manBool) {
        ...
        CCMenuItem* quedingItem = CCMenuItemSprite::create(qdSpr1, qdSpr2, this, menu_selector(NXClothesScene::tuoguangConfirmCallBack));
        ...
        CCMenuItem* fanhuiItem = CCMenuItemSprite::create(fhSpr1, fhSpr2, this, menu_selector(NXClothesScene::tuoguangCancelCallBack));
...
void NXClothesScene::renwukuangCallBack(CCObject* pSender){
    AUDIO->common_effect();
...
void NXClothesScene::buttonCallBack(CCObject* pSender){
    AUDIO->common_effect();
...
void NXClothesScene::guideButtonCallBack(){
    if (animationBool) {
        animationBool = false;
...
void NXClothesScene::_605CallBack(CCObject* pObj){
    LOADING->remove();
...
void NXClothesScene::backCallBack(CCObject* pSender){
    AUDIO->goback_effect();
...
void NXClothesScene::_905CallBack(CCObject* pObj){
    LOADING->remove();
...
void NXClothesScene::after_commit_mystery_613(CCObject *pObj) {
...
void NXClothesScene::after_start_tryst_task_623(CCObject *pObj) {
    CCScene* scene = NXMainScene::scene();
    CCTransitionFade* trans = CCTransitionFade::create(0.6, scene);
    CCDirector::sharedDirector()->replaceScene(trans);
}

void NXClothesScene::removeAnimation(){
    if (animationBool) {
        animationBool = false;
...
void NXClothesScene::startCallBack(CCObject* pSender){
    AUDIO->common_effect();
    ...
    if (syClothesId != 30000) {
        CCArray* arr = (CCArray* )allClothesDic->objectForKey(Tag_NXCL_ShangYi);// 获得当前类型所有衣服
    ...
    if (!allClothesBool) {// 全都没穿
        ...
        if (syClothesId != Tag_NXCL_ShangYi* 10000) {
            if (sySub_part == 0) {
                if (kzClothesId == Tag_NXCL_KuZi* 10000) {
                    PromptLayer* layer = PromptLayer::create();
                    layer->show_prompt(this->getScene(), "喂喂喂~!你是不是忘记穿裤子了");
                    return;
                }else if (xzClothesId == Tag_NXCL_XieZi * 10000){
                    PromptLayer* layer = PromptLayer::create();
                ...
                if (xzClothesId == Tag_NXCL_XieZi * 10000) {
                    PromptLayer* layer = PromptLayer::create();
                    layer->show_prompt(this->getScene(), "鞋子都不穿,就不怕扎脚么....");
                }else{
                    this->startMethods();
...
void NXClothesScene::startMethods(){
    CCDictionary* allClothesDic = CONFIG->clothes();// 所有衣服
    CCDictionary* myClothesTempDic = DATA->getClothes()->MyClothesTemp();
...
void NXClothesScene::buyCallBack(CCObject* pSender){
    AUDIO->common_effect();
...
void NXClothesScene::saveCallBack(CCObject* pSender){
    AUDIO->common_effect();
...
void NXClothesScene::updataSaveItemStatus(){
    if (clothesStatus == 2 || clothesStatus == 5 || clothesStatus == 6){// 换装
        saveItem->setEnabled(true);
    }
}
void NXClothesScene::saveClothesMethods(){
    buyClothesStr = "";
...
void NXClothesScene::ChangeShipin(int clothesId, int sub_part){
    CCDictionary* dic = CONFIG->clothes();// 所有衣服
    CCTextureCache::sharedTextureCache()->removeUnusedTextures();
    CCArray* clothesArr = (CCArray* )dic->objectForKey(Tag_NXCL_ShiPin);// 获得当前类型所有衣服
...
void NXClothesScene::initClothes(){//穿衣服
    ...
    this->scheduleOnce(SEL_SCHEDULE(&NXClothesScene::openTouch), 1.f);
}
void NXClothesScene::ChangClothesIndex(CCObject* pSender){
...
void NXClothesScene::ChangeClothes(CCObject* pSender){
    ...
    switch (isClothesType) {
        case Tag_NXCL_TouFa:{
            if (_ManSpr->getChildByTag(Tag_NXCL_TouFa1) != NULL) {
                _ManSpr->removeChildByTag(Tag_NXCL_TouFa1);
            }
            if (_ManSpr->getChildByTag(Tag_NXCL_TouFa2) != NULL) {
                _ManSpr->removeChildByTag(Tag_NXCL_TouFa2);
            }
            if (_ManSpr->getChildByTag(Tag_NXCL_TouFa3) != NULL) {
                _ManSpr->removeChildByTag(Tag_NXCL_TouFa3);
            }
            CCTextureCache::sharedTextureCache()->removeUnusedTextures();
        ...
        case Tag_NXCL_WaiTao:{
            if (_ManSpr->getChildByTag(Tag_NXCL_WaiTao1) != NULL) {
                _ManSpr->removeChildByTag(Tag_NXCL_WaiTao1);
            }
            if (_ManSpr->getChildByTag(Tag_NXCL_WaiTao2) != NULL) {
                _ManSpr->removeChildByTag(Tag_NXCL_WaiTao2);
            }
            if (_ManSpr->getChildByTag(Tag_NXCL_WaiTao3) != NULL) {
                _ManSpr->removeChildByTag(Tag_NXCL_WaiTao3);
            }
            CCTextureCache::sharedTextureCache()->removeUnusedTextures();
        ...
        case Tag_NXCL_ShangYi:{
            if (_ManSpr->getChildByTag(Tag_NXCL_ShangYi1) != NULL) {
                _ManSpr->removeChildByTag(Tag_NXCL_ShangYi1);
            }
            if (_ManSpr->getChildByTag(Tag_NXCL_ShangYi2) != NULL) {
                _ManSpr->removeChildByTag(Tag_NXCL_ShangYi2);
            }
            if (_ManSpr->getChildByTag(Tag_NXCL_ShangYi2) != NULL) {
                _ManSpr->removeChildByTag(Tag_NXCL_ShangYi2);
            }
            ...
            if (index == 30000) {
                ...
                if (kuziBool) {
                    ...
                    if (_ManSpr->getChildByTag(Tag_NXCL_KuZi1) != NULL) {
                        _ManSpr->removeChildByTag(Tag_NXCL_KuZi1);
                    }
                    if (_ManSpr->getChildByTag(Tag_NXCL_KuZi2) != NULL) {
                        _ManSpr->removeChildByTag(Tag_NXCL_KuZi2);
                    }
                ...
                for (int j = 0; j < clothesArr->count(); j++) {
                    ...
                    if (now_clothes_Id == index) {
                        ...
                        if (sub_part == 1) {
                            ...
                            CCInteger* clothesTemp_id = (CCInteger* )clothesTemp->objectForKey(CCString::createWithFormat("%d", Tag_NXCL_KuZi)->getCString());
        ...
        case Tag_NXCL_XieZi:{
            if (_ManSpr->getChildByTag(Tag_NXCL_XieZi1) != NULL) {
                _ManSpr->removeChildByTag(Tag_NXCL_XieZi1);
            }
            if (_ManSpr->getChildByTag(Tag_NXCL_XieZi2) != NULL) {
                _ManSpr->removeChildByTag(Tag_NXCL_XieZi2);
            }
            if (_ManSpr->getChildByTag(Tag_NXCL_XieZi3) != NULL) {
                _ManSpr->removeChildByTag(Tag_NXCL_XieZi3);
            }
            CCTextureCache::sharedTextureCache()->removeUnusedTextures();
        ...
        case Tag_NXCL_Bao:{
            if (_ManSpr->getChildByTag(Tag_NXCL_Bao1) != NULL) {
                _ManSpr->removeChildByTag(Tag_NXCL_Bao1);
            }
            if (_ManSpr->getChildByTag(Tag_NXCL_Bao2) != NULL) {
                _ManSpr->removeChildByTag(Tag_NXCL_Bao2);
            }
            if (_ManSpr->getChildByTag(Tag_NXCL_Bao3) != NULL) {
                _ManSpr->removeChildByTag(Tag_NXCL_Bao3);
            }
            if (_ManSpr->getChildByTag(Tag_NXCL_Bao4) != NULL) {
                _ManSpr->removeChildByTag(Tag_NXCL_Bao4);
            }
            CCTextureCache::sharedTextureCache()->removeUnusedTextures();
        ...
        case Tag_NXCL_ZhuangRong:{
            if (_ManSpr->getChildByTag(Tag_NXCL_ZhuangRong1) != NULL) {
                _ManSpr->removeChildByTag(Tag_NXCL_ZhuangRong1);
            }
            CCTextureCache::sharedTextureCache()->removeUnusedTextures();
...
CCSprite* NXClothesScene::creat_Gray(int type, int index, CCString* str){
...
void NXClothesScene::reductionShangyi(){
    if (_ManSpr->getChildByTag(Tag_NXCL_ShangYi1) != NULL) {
        _ManSpr->removeChildByTag(Tag_NXCL_ShangYi1);
    }
    if (_ManSpr->getChildByTag(Tag_NXCL_ShangYi2) != NULL) {
...
void NXClothesScene::reductionShipin(int index){
...
void NXClothesScene::buttonStatus(){
//    CCDictionary* clothesTemp = DATA->getClothes()->MyClothesTemp(); // 临时数组
//    
//    for (int i = Tag_NXCL_TouFa; i <= Tag_NXCL_ZhuangRong; i++) {
//        if (i == Tag_NXCL_TouFa) {
...
void NXClothesScene::Http_Finished_401(cocos2d::CCObject *pObj) {
...
//void NXClothesScene::Http_Finished_403(cocos2d::CCObject *pObj) {
//    this->Http_Finished_401(pObj);
//}

void NXClothesScene::create_buySuccess(){
    animationBool = true;
...
void NXClothesScene::removeAllSpr(){
...
int NXClothesScene::getTaskId(int index){
    CCArray* taskArr = DATA->getTaskSource();
...
void NXClothesScene::go_fitting_room() {
...
//void NXClothesScene::Http_Finished_601(CCObject* pObj){
//    int tili = DATA->getPlayer()->energy;
//    tili_AllIndex = 9;
//    if (tili >= tili_AllIndex) {
//        NET->commit_mission_603(getTaskId(task_index - 1));
...
void NXClothesScene::Http_Finished_603(CCObject* pObj){
...
void NXClothesScene::message_box_did_selected_button(AHMessageBox* box, AH_BUTTON_TYPE button_type, AH_BUTTON_TAGS button_tag){
    box->animation_out();
...
int NXClothesScene::haveEnoughCoin(){
    int coin = 0;
...
int NXClothesScene::haveEnoughGold(){
    int gold = 0;
...
int NXClothesScene::haveEnoughDebris(){
    int debris = 0;
...
void NXClothesScene::setShipinTag1(int index, CCSprite* spr){
    if (index == 11) {
        spr->setTag(Tag_NXCL_ShiPin1_1);
    }else if (index == 12){
        spr->setTag(Tag_NXCL_ShiPin1_2);
    }else if (index == 13){
        spr->setTag(Tag_NXCL_ShiPin1_3);
    }else if (index == 14){
        spr->setTag(Tag_NXCL_ShiPin1_4);
    }else if (index == 15){
        spr->setTag(Tag_NXCL_ShiPin1_5);
    }else if (index == 16){
        spr->setTag(Tag_NXCL_ShiPin1_6);
    }else if (index == 17){
        spr->setTag(Tag_NXCL_ShiPin1_7);
    }else if (index == 18){
        spr->setTag(Tag_NXCL_ShiPin1_8);
    }else if (index == 19){
        spr->setTag(Tag_NXCL_ShiPin1_9);
    }
}
void NXClothesScene::setShipinTag2(int index, CCSprite* spr){
    if (index == 11) {
        spr->setTag(Tag_NXCL_ShiPin2_1);
    }else if (index == 12){
        spr->setTag(Tag_NXCL_ShiPin2_2);
    }else if (index == 13){
        spr->setTag(Tag_NXCL_ShiPin2_3);
    }else if (index == 14){
        spr->setTag(Tag_NXCL_ShiPin2_4);
    }else if (index == 15){
        spr->setTag(Tag_NXCL_ShiPin2_5);
    }else if (index == 16){
        spr->setTag(Tag_NXCL_ShiPin2_6);
    }else if (index == 17){
        spr->setTag(Tag_NXCL_ShiPin2_7);
    }else if (index == 18){
        spr->setTag(Tag_NXCL_ShiPin2_8);
    }else if (index == 19){
        spr->setTag(Tag_NXCL_ShiPin2_9);
    }
}
void NXClothesScene::setShipinTag3(int index, CCSprite* spr){
    if (index == 11) {
        spr->setTag(Tag_NXCL_ShiPin3_1);
    }else if (index == 12){
        spr->setTag(Tag_NXCL_ShiPin3_2);
    }else if (index == 13){
        spr->setTag(Tag_NXCL_ShiPin3_3);
    }else if (index == 14){
        spr->setTag(Tag_NXCL_ShiPin3_4);
    }else if (index == 15){
        spr->setTag(Tag_NXCL_ShiPin3_5);
    }else if (index == 16){
        spr->setTag(Tag_NXCL_ShiPin3_6);
    }else if (index == 17){
        spr->setTag(Tag_NXCL_ShiPin3_7);
    }else if (index == 18){
        spr->setTag(Tag_NXCL_ShiPin3_8);
    }else if (index == 19){
        spr->setTag(Tag_NXCL_ShiPin3_9);
    }
}

int NXClothesScene::updataClothes(int type){
    return (type) * 10000;
}

void NXClothesScene::showAnimationWithType(int type) {
...
void NXClothesScene::buyClothesMethods(){
    CCDictionary* dic = CONFIG->clothes();// 所有衣服
