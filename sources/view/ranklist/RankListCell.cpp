...
void RankListCell::configCompetition(int idx, CompetitionItem *item, float cellWidth, float cellHeight) {
    _idx = idx;
    ...
    if (item->getId().compare(DATA->getLogin()->obtain_sid()) == 0) {
        //这是我自己
        
    }else {
        if (DATA->getSocial()->is_friend(item->getId().c_str()) || item->getAdded() == true) {
            //是好友
...
void RankListCell::configShower(int idx, ShowComp *show, float cellWidth, float cellHeight) {
    _idx = idx;
    float halfCellWidth = cellWidth * 0.5;
    float halfCellHeight = cellHeight * 0.5;
